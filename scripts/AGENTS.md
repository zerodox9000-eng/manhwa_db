...
- `build/rebuildUpdatesExport.js` refreshes only the compact Updates payload from the currently committed full chunked catalogue/history plus the update ledger. Use it for focused validation without rerunning the full frontend export.
...
- The chunk manifest retains full `history` for currently deployed clients and adds `weeklyHistory` with only the observations needed for the exact latest rolling week. New clients prefer the compact dataset and fall back to full history. Recommendation generation is suspended: the builder skips recommendation context/ranking work and publishes only an empty compatibility dataset so existing manifest readers do not fail.
